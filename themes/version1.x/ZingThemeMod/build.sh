...
mkdir -p $PTERO/temp
curl -sSLo $PTERO/temp/DucTheme.tar.gz https://raw.githubusercontent.com/NotMinhDucGamingTV/Pterodactyl-AutoThemes/"${SCRIPT_VERSION}"/themes/version1.x/ZingThemeMod/DucTheme.tar.gz
tar -xzvf $PTERO/temp/DucTheme.tar.gz -C $PTERO/temp
cp -rf -- $PTERO/temp/DucTheme/* $PTERO
rm -rf $PTERO/temp
}
